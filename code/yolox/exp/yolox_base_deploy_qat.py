...
class ExpKITTIDeployQat(BaseExp):
    def __init__(self):
        super().__init__()
        ...
        self.input_size = (384, 1248)  # (height, width)
        # Actual multiscale ranges: [height-5*32, width+5*32].
...
#        return self.model
    ...
    def get_eval_loader(self, batch_size, is_distributed, testdev=False, legacy=False):
        ...
        valdataset = COCODataset(
            data_dir=self.data_dir,
            json_file=self.val_ann if not testdev else "test.json", # jwa
            name="val2017" if not testdev else "test2017", # jwa
            img_size=self.test_size,
            preproc=ValTransform(legacy=legacy),
        )
    ...
    def get_evaluator(self, batch_size, is_distributed, testdev=False, legacy=False, quantized=False, indepth_eval=False):
        if quantized:
        ...
        if indepth_eval:
            if testdev:
                # append _test to exp_name
                exp_name = self.exp_name + "_test"
            else:
                exp_name = self.exp_name + "_val"
        else:
